MININIX_PKG_HOMEPAGE=https://www.gnu.org/software/bash/
MININIX_PKG_DESCRIPTION="A sh-compatible shell that incorporates useful features from the Korn shell (ksh) and C shell (csh)"
MININIX_PKG_DEPENDS="ncurses, readline, libandroid-support, mininix-tools, command-not-found"
_MAIN_VERSION=4.4
_PATCH_VERSION=23
MININIX_PKG_REVISION=3
MININIX_PKG_SHA256=d86b3392c1202e8ff5a423b302e6284db7f8f435ea9f39b5b1b20fd3ac36dfcb
MININIX_PKG_VERSION=${_MAIN_VERSION}.${_PATCH_VERSION}
MININIX_PKG_SRCURL=https://mirrors.kernel.org/gnu/bash/bash-${_MAIN_VERSION}.tar.gz
MININIX_PKG_ESSENTIAL=true

MININIX_PKG_EXTRA_CONFIGURE_ARGS="--enable-multibyte --without-bash-malloc --with-installed-readline ac_cv_header_grp_h=no ac_cv_rl_version=7.0"
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_job_control_missing=present"
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_sys_siglist=yes"
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_func_sigsetjmp=present"
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_unusable_rtsigs=no"
...
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_dev_fd=whacky"
...
# - https://github.com/mininix/mininix-app/issues/200
MININIX_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_getcwd_malloc=yes"

MININIX_PKG_RM_AFTER_INSTALL="share/man/man1/bashbug.1 bin/bashbug"
...
mininix_step_pre_configure () {
	...
	for patch_number in `seq -f '%03g' ${_PATCH_VERSION}`; do
		PATCHFILE=$MININIX_PKG_CACHEDIR/bash_patch_${patch_number}.patch
		mininix_download \
...
mininix_step_post_make_install () {
	sed "s|@MININIX_PREFIX@|$MININIX_PREFIX|" $MININIX_PKG_BUILDER_DIR/etc-profile > $MININIX_PREFIX/etc/profile
	sed "s|@MININIX_PREFIX@|$MININIX_PREFIX|" \
		$MININIX_PKG_BUILDER_DIR/etc-profile | \
		sed "s|@MININIX_HOME@|$MININIX_ANDROID_HOME|" > \
		$MININIX_PREFIX/etc/profile
	# /etc/bash.bashrc - System-wide .bashrc file for interactive shells. (config-top.h in bash source, patched to enable):
	sed "s|@MININIX_PREFIX@|$MININIX_PREFIX|" \
		$MININIX_PKG_BUILDER_DIR/etc-bash.bashrc > \
		$MININIX_PREFIX/etc/bash.bashrc
}
