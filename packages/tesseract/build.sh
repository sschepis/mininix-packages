LINUXDROID_PKG_HOMEPAGE=https://github.com/tesseract-ocr/tesseract
LINUXDROID_PKG_DESCRIPTION="Tesseract is probably the most accurate open source OCR engine available"
LINUXDROID_PKG_VERSION=3.05.02
LINUXDROID_PKG_REVISION=1
LINUXDROID_PKG_SHA256=494d64ffa7069498a97b909a0e65a35a213989e0184f1ea15332933a90d43445
LINUXDROID_PKG_SRCURL=https://github.com/tesseract-ocr/tesseract/archive/${LINUXDROID_PKG_VERSION}.tar.gz
LINUXDROID_PKG_DEPENDS="libtool, libuuid, leptonica"

linuxdroid_step_pre_configure() {
	# http://blog.matt-swain.com/post/26419042500/installing-tesseract-ocr-on-mac-os-x-lion
	export LIBLEPT_HEADERSDIR=${LINUXDROID_PREFIX}/include/leptonica
...
linuxdroid_step_post_make_install() {
	...
	cd "${LINUXDROID_PREFIX}/share/tessdata"
	rm -f eng.*
	...
	mkdir -p $LINUXDROID_PKG_CACHEDIR/tessdata
	...
	for f in cube.{bigrams,fold,lm,nn,params,size,word-freq} tesseract_cube.nn traineddata; do
		...
		linuxdroid_download \
			...
			$LINUXDROID_PKG_CACHEDIR/tessdata/eng.$f \
			${checksums[$f]}
		cp $LINUXDROID_PKG_CACHEDIR/tessdata/eng.$f .
	done
}
