LINUXDROID_PKG_HOMEPAGE=http://python.org/
LINUXDROID_PKG_DESCRIPTION="Python 2 programming language intended to enable clear programs"
...
LINUXDROID_PKG_DEPENDS="libandroid-support, ncurses, readline, libffi, openssl, libutil, libbz2, libsqlite, gdbm, ncurses-ui-libs, libcrypt"
# Python.h includes crypt.h:
LINUXDROID_PKG_DEVPACKAGE_DEPENDS="libcrypt-dev"
LINUXDROID_PKG_HOSTBUILD=true

_MAJOR_VERSION=2.7
LINUXDROID_PKG_VERSION=${_MAJOR_VERSION}.15
LINUXDROID_PKG_REVISION=4
LINUXDROID_PKG_SHA256=22d9b1ac5b26135ad2b8c2901a9413537e08749a753356ee913c84dbd2df5574
...
LINUXDROID_PKG_EXTRA_CONFIGURE_ARGS+=" --enable-unicode=ucs4"
...
linuxdroid_step_host_build () {
	# We need a host-built Parser/pgen binary, copied into cross-compile build in linuxdroid_step_post_configure() below
...
linuxdroid_step_post_configure () {
	cp $LINUXDROID_PKG_HOSTBUILD_DIR/Parser/pgen $LINUXDROID_PKG_BUILDDIR/Parser/pgen
	touch -d "next hour" $LINUXDROID_PKG_BUILDDIR/Parser/pgen
}

linuxdroid_step_pre_configure() {
	# Put the host-built python in path:
	export LINUXDROID_ORIG_PATH=$PATH
	export PATH=$LINUXDROID_PKG_HOSTBUILD_DIR:$PATH
...
linuxdroid_step_post_make_install () {
	...
	(cd $LINUXDROID_PREFIX/bin
	 ...
	 ln -sf pydoc${_MAJOR_VERSION} pydoc2)
        # Restore path which linuxdroid_step_host_build messed with
        export PATH=$LINUXDROID_ORIG_PATH
}
...
linuxdroid_step_create_debscripts () {
	...
	echo "if [ ! -f $LINUXDROID_PREFIX/bin/pip2 -a -d $LINUXDROID_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ]; then rm -Rf $LINUXDROID_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ; fi" >> postinst
	# Setup bin/pip2:
	echo "$LINUXDROID_PREFIX/bin/python2 -m ensurepip --upgrade --no-default-pip" >> postinst
	...
	# Cleanup *.pyc files
	echo "find $LINUXDROID_PREFIX/lib/python${_MAJOR_VERSION} -depth -name *.pyc -exec rm -rf {} +" >> prerm
	...
	echo "rm -f $LINUXDROID_PREFIX/bin/pip2* $LINUXDROID_PREFIX/bin/easy_install-2*" >> prerm
