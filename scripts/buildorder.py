...
def parse_build_file_dependencies(path):
    ...
    pkg_dep_prefix = 'MININIX_PKG_DEPENDS='
    pkg_build_dep_prefix = 'MININIX_PKG_BUILD_DEPENDS='
    subpkg_dep_prefix = 'MININIX_SUBPKG_DEPENDS='
    dependencies = []
    ...
    def __init__(self, dir_path):
        ...
        for filename in os.listdir(self.dir):
            if not filename.endswith('.subpackage.sh'):
                continue
            subpkg = MininixSubPackage(self.dir + '/' + filename, self)
...
def read_packages_from_directories(directories):
    """Construct a map from package name to MininixPackage.
    ...
    for package_dir in directories:
        for pkgdir_name in sorted(os.listdir(package_dir)):
            ...
            if os.path.isfile(dir_path + '/build.sh'):
                new_package = MininixPackage(package_dir + '/' + pkgdir_name)
    ...
    for pkg in all_packages:
        for dependency_name in pkg.deps:
            ...
            if not isinstance(pkg, MininixSubPackage):
...
def generate_full_buildorder(pkgs_map):
    ...
    # List of all MininixPackages without dependencies
