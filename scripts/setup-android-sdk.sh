...
ANDROID_NDK_FILE=android-ndk-r${LINUXDROID_NDK_VERSION}-Linux-x86_64.zip
...
if [ ! -d $NDK ]; then
	...
	rm -Rf android-ndk-r$LINUXDROID_NDK_VERSION
	unzip -q ndk.zip
	mv android-ndk-r$LINUXDROID_NDK_VERSION `basename $NDK`
	rm ndk.zip
fi
...
$ANDROID_HOME/tools/bin/sdkmanager "build-tools;${LINUXDROID_ANDROID_BUILD_TOOLS_VERSION}" "platforms;android-27" "platforms;android-21"
