...
def validate_package(package_name, lines):
    if len(lines) < 3:
        ...
        return
    if not lines[0].startswith('LINUXDROID_PKG_HOMEPAGE='):
        print('The first line is not LINUXDROID_PKG_HOMEPAGE: ' + package_name)
    if not lines[1].startswith('LINUXDROID_PKG_DESCRIPTION='):
        print('The second line is not LINUXDROID_PKG_DESCRIPTION: ' + package_name)

    line_number = 1
    for line in lines:
        if line.endswith(' \n'):
            print(package_name + ': Line ' + str(line_number) + ' has trailing whitespace')
        if line.startswith('LINUXDROID_PKG_REVISION='):
            value = line[len('LINUXDROID_PKG_REVISION='):].strip()
            if not re.match('[0-9]+', value):
                print(package_name + ': strange LINUXDROID_PKG_REVISION value "' + value + '"')
